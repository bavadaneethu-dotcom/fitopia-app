    ...
    <div className="relative flex h-full min-h-screen w-full flex-col overflow-hidden bg-white dark:bg-[#1a1a1a] text-light-text dark:text-white font-sans transition-colors duration-300">
      ...
      <div className="flex flex-col items-center w-full pt-8 px-6 relative z-20 bg-white dark:bg-[#1a1a1a]">
        <div className="w-full flex items-center justify-between mb-4">
            ...
            <div className="px-4 py-1.5"><span className="text-[10px] font-black text-indigo-400 dark:text-indigo-300 uppercase tracking-[0.2em]">Step 3 of 5</span></div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-6 pb-44">
        <div className="py-2 mt-4 mb-6">
          <h1 className="text-[34px] font-black text-center text-[#4A4A4A] dark:text-white leading-[0.95] tracking-tight mb-6">Choose your <br/> <span className="text-[#6D5D4B] dark:text-gray-400">mission</span></h1>
          <div className="flex gap-4 bg-[#F9F9F9] dark:bg-white/5 p-5 rounded-2xl border border-gray-100 dark:border-white/5 shadow-sm">
             ...
             <p className="text-sm font-bold leading-relaxed text-gray-600 dark:text-gray-300 italic">"{activeCharacter?.onboardingMessages.goal || "So, what's the angle? Looking good or feeling good? I can help with both."}"</p>
          </div>
        </div>

        <div className="flex flex-col gap-4">
          <div onClick={() => setUserGoal('lose')} className={`group relative cursor-pointer flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-300 ease-out ${userGoal === 'lose' ? 'border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 shadow-md scale-[1.02]' : 'border-transparent bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10'}`}>
            ...
            <div className="flex flex-col flex-grow min-w-0">
              <div className="flex justify-between items-start mb-1">
                ...
                <div className={`size-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${userGoal === 'lose' ? 'border-yellow-400 bg-yellow-400' : 'border-gray-300 bg-transparent'}`}>{userGoal === 'lose' && <span className="material-symbols-outlined text-black text-[16px] font-bold">check</span>}</div>
              </div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Get lean and agile.</p>
              <div className="flex items-center gap-1 text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-md w-fit uppercase tracking-wider">High Intensity</div>
            </div>
          </div>

          <div onClick={() => setUserGoal('maintain')} className={`group relative cursor-pointer flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-300 ease-out ${userGoal === 'maintain' ? 'border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 shadow-md scale-[1.02]' : 'border-transparent bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10'}`}>
            ...
            <div className="flex flex-col flex-grow min-w-0">
              <div className="flex justify-between items-start mb-1">
                ...
                <div className={`size-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${userGoal === 'maintain' ? 'border-yellow-400 bg-yellow-400' : 'border-gray-300 bg-transparent'}`}>{userGoal === 'maintain' && <span className="material-symbols-outlined text-black text-[16px] font-bold">check</span>}</div>
              </div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Stay fit and healthy.</p>
              <div className="flex items-center gap-1 text-[10px] font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded-md w-fit uppercase tracking-wider">Balanced Routine</div>
            </div>
          </div>

          <div onClick={() => setUserGoal('gain')} className={`group relative cursor-pointer flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-300 ease-out ${userGoal === 'gain' ? 'border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 shadow-md scale-[1.02]' : 'border-transparent bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10'}`}>
            ...
            <div className="flex flex-col flex-grow min-w-0">
              <div className="flex justify-between items-start mb-1">
                ...
                <div className={`size-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${userGoal === 'gain' ? 'border-yellow-400 bg-yellow-400' : 'border-gray-300 bg-transparent'}`}>{userGoal === 'gain' && <span className="material-symbols-outlined text-black text-[16px] font-bold">check</span>}</div>
              </div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Build strength & power.</p>
              <div className="flex items-center gap-1 text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md w-fit uppercase tracking-wider">Strength Focus</div>
            </div>
          </div>
        </div>
      </div>
