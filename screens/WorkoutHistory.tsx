...
const WorkoutHistory: React.FC<WorkoutHistoryProps> = ({ logs, onNavigate }) => {
  ...
  useEffect(() => {
    if (logs.length === 0) {
      // Reset state if re-entering
      setText1("");
      setShowPart2(false);

      let index = 0;
      // Delay start slightly to allow "Dismissed" stamp to land first
      const startDelay = setTimeout(() => {
          const interval = setInterval(() => {
            ...
            if (index > quotePart1.length) {
                ...
                setTimeout(() => setShowPart2(true), 400); // Pause before shouting command
            }
          }, 40); // Typing speed
  ...
  return (
    <div className="relative flex h-full min-h-screen w-full flex-col overflow-hidden bg-light-bg dark:bg-dark-bg animate-fade-in font-sans">
          ...
          <div className="flex-1 flex flex-col items-center justify-center min-h-[65vh]">
             ...
             <div className="relative w-64 h-[340px] border-2 border-[#6ba5ff] dark:border-blue-400/50 flex flex-col items-center mb-8 animate-scan-in bg-blue-50/10 backdrop-blur-[2px]">
                ...
                <div className="relative mb-auto">
                    ...
                    {/* Donut */}
                    <div className="relative z-10 text-[100px] animate-float drop-shadow-xl">
                        🍩
                    </div>
                    
                    {/* Runner on top */}
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 text-4xl animate-run-in-place">
                        🏃
                    </div>
                </div>

                {/* Corners for Blueprint feel */}
             ...
             <div className="text-center max-w-xs space-y-3 z-10">
                 <h1 className="text-3xl font-black italic text-[#3F2E23] dark:text-white tracking-tighter transform -rotate-2 drop-shadow-sm animate-stamp origin-center">
                    "DISMISSED!"
                 </h1>
                        ...
                        {showPart2 && (
                             <>
                                ...
                                <button 
                                  ...
                                  className="text-[#F97316] font-black uppercase animate-pop-in inline-block mt-2 hover:underline cursor-pointer"
                                >
                                  START TRAINING!
                                </button>"
                             </>
                 ...
                 <div className="flex items-center justify-center gap-2 mt-4 opacity-0 animate-fade-in delay-[3000ms] fill-mode-forwards">
                     <div className="h-px w-6 bg-yellow-400"></div>
                     <p className="text-[10px] font-black text-yellow-500 uppercase tracking-[0.2em]">Chief Bogo</p>
                     <div className="h-px w-6 bg-yellow-400"></div>
                 </div>
             </div>
          ...
          <div className="flex flex-col gap-4 pb-20">
             ...
             {logs.map((log, idx) => (
                <div key={log.id} className="flex items-center gap-4 p-4 bg-white dark:bg-white/5 rounded-[1.5rem] border border-gray-100 dark:border-white/5 shadow-sm animate-fade-in-up hover:scale-[1.02] transition-transform" style={{ animationDelay: `${idx * 50}ms` }}>
                    <div className={`size-14 rounded-2xl flex items-center justify-center text-3xl shadow-inner ${log.color ? `bg-${log.color}-100 text-${log.color}-600 dark:bg-${log.color}-900/20 dark:text-${log.color}-300` : 'bg-orange-100 text-orange-600'}`}>
                        {log.icon}
                    </div>
      ...
      <style>{`
        ...
        .animate-scan-in {
            animation: scan-in 0.8s cubic-bezier(0.25, 1, 0.5, 1) forwards;
        }
        ...
        .animate-stamp {
            animation: stamp 0.5s cubic-bezier(0.2, 0.8, 0.2, 1) 0.6s backwards;
        }
        
        @keyframes run-in-place {
            0%, 100% { transform: translate(-50%, 0) rotate(0deg); }
            25% { transform: translate(-50%, -6px) rotate(5deg); }
            50% { transform: translate(-50%, 0) rotate(0deg); }
            75% { transform: translate(-50%, -6px) rotate(-5deg); }
        }
        .animate-run-in-place {
            animation: run-in-place 0.5s infinite linear;
        }
